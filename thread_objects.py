...
class trdGenData(QThread):
    res_to_emit_status = pyqtSignal(list, list, str, str, str, bool)
    ...
    def run(self):
        ...
        if openfile_obj['cut_off'] == '1':
            ...
            dataInfoJson = [{'cut_off': str(openfile_obj['cut_off']), 'month': str(openfile_obj['date_to_1']['month']), 'year': str(openfile_obj['date_to_1']['year']), 'SCName': openfile_obj['SCName']}]
            ...
            dataInfoJson = [{'cut_off': str(openfile_obj['cut_off']), 'month': str(openfile_obj['date_from_2']['month']), 'year': str(openfile_obj['date_from_2']['year']), 'SCName': openfile_obj['SCName']}]
        ...
        try:
            ...
            if res2 != []:#dili ma save ang new date pag blank ang query
                ...
                self.res_to_emit_status.emit(res2, dataInfoJson, 'Succesfully Generated', 'color: green', genDate, True)
            else:
                genDate = f'From {dateFrom_fmt} To {dateTo_fmt}'
                self.res_to_emit_status.emit(res2, dataInfoJson, 'Error in Generating', 'color: red', genDate, False)
    ...
    def run(self):
        try:
            ...
            "data": {
                ...
                "data_inf_json": json.dumps(self.list_gen_info)
            }
            })
